%{
    ...
    the PSO optimization algorithm to solve the planning problem.
...
for times_ind = 1:test_time
    ...
    for i_router = 1:num_router
        ...
        %{
            If you need to test other parameter groups or other 
            initialization methods of the PSO algorithm, 
            please modify the following code.
            DON'T forget to synchronously modify the PSO configuration for
            the gateway below. 
        %}
    ...
    for i_gateway = 1:gateway_num
        ...
        % Calculate the maximum wireless transmission loss before optimization
        loss_before = pos_fitness4ps0(router_pos_self, ...
            gateway_position(:,i_gateway),lamda,@terrain, ...
            search_len,@KED_fitness);
        % Get the center of the initialization algorithm
        ...
        [pos_res,foo] = particleswarm(@(x)...
            (pos_fitness4ps0(router_pos_self,x,lamda,@terrain,search_len, ...
            @KED_fitness)),2,[border(1),border(1)], ...
            [border(2),border(2)],options4pso);
...
save('./exc_res/PSO_rand_circle_1.mat',"fitness_router_rec","fitness_gateway_rec","time_rec");
